define([
    "dojo/_base/declare", "dojo/dnd/move", "dijit/_WidgetBase", "dijit/_TemplatedMixin", "dojo/text!./templates/basemapSelector.html",
    "dojo/query", "dojo/dom", "dojo/dom-class", "dojo/dom-style","dojo/dom-construct", "dojo/dom-attr",  "dojo/on", "dojo/touch", "dojox/gesture/tap",
    "dojo/NodeList-traverse", "dojo/NodeList-manipulate"
 ], function (declare, move, _WidgetBase, _TemplatedMixin, template, query,
    dom, domClass, domStyle, domConstruct, domAttr, on, touch, tap) {
...
basemapChanger: function() {
    var i = dom.byId("imagelist2");
        if (!(i.style.display === 'block')) {
            ...
            thisWidget.device === "desktop"? (domClass.replace(dom.byId("DTbasemap"), "DTimagery", "DTbasemap")):
            (domClass.replace(dom.byId("MBasemap"), "Mimagery", "Mbasemap"));
            domAttr.set(dom.byId("DTbasemap"), 'title', "Turn on Default Basemap");
            domAttr.set(dom.byId("DTbasemap"), 'title', "Turn on Default Basemap");
        } else {
            i.style.display = "none";
            thisWidget.loadYear('vector');
            thisWidget.device === "desktop"? (domClass.replace(dom.byId("DTbasemap"), "DTbasemap", "DTimagery")):
            (domClass.replace(dom.byId("MBasemap"), "Mbasemap", "Mimagery"));
            domAttr.set(dom.byId("DTbasemap"), 'title', "Turn on Imagery");
        }
},
