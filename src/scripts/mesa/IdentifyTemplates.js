    ...
    return {
        ...
        subList: function (evt, deferred,len, map) {
            ...
            deferred.addCallback(function (response) {
                ...
                for (i = 0; i < len; i++) {
                  //The layerName is the name of the layer in the mxd that powers the service
                    if (contentArray[i].layerName === "Subdivision Plats") {
                        att = contentArray[i].feature.attributes;
                        content += "<span title='Subdivision - Click to highlight' class='nored sub' id='i" + att.OBJECTID + "' onclick='getit(" + att.OBJECTID + ",19);' " +
                    ...
                    } else if (contentArray[i].layerName === "Dedicated ROW (2007-2017)") {
                        att = contentArray[i].feature.attributes;
                        content += "<span title='Dedicated/Platted ROW - Click to highlight' class='nored ded' id='i" + att.OBJECTID + "' onclick='getit(" + att.OBJECTID + ",15);'></span> " + "<a target='_blank' href='https://recording.mesacounty.us/Landmarkweb//search/DocumentBy?ClerkFileNumber=" + att.Reception + "'><b>" + att['Road Name'] + "</b> (Dedicated/Platted ROW)</a>";
                    } else if (contentArray[i].layerName === "Vacated ROW") {
                        att = contentArray[i].feature.attributes;
                        content += "<span title='Vacated ROW - Click to highlight' class='nored vac' id='i" + att.OBJECTID + "' onclick='getit(" + att.OBJECTID + ",14);' " + "></span> " + "<a target='_blank' href='https://recording.mesacounty.us/Landmarkweb//search/DocumentBy?ClerkFileNumber=" + att.RECEPTION + "'><b>" + att.ROAD + "</b> (Vacated ROW)</a><br>Book: " + att.BOOK + "<br>Page: " + att.PAGE + "<br>";
                    } else if (contentArray[i].layerName === "Permanant Easement - ROW") {
                        att = contentArray[i].feature.attributes;
                        content += "<span title='Permanent Easement ROW - Click to highlight' class='nored ease' id='i" + att.OBJECTID + "' onclick='getit(" + att.OBJECTID + ",18);'></span> " + "<a target='_blank' href='https://recording.mesacounty.us/Landmarkweb//search/DocumentBy?ClerkFileNumber=" + att.RECEPTION + "'><b>" + att.ROAD + "</b> (Permanent Easement ROW)</a>";
                    } else if (contentArray[i].layerName === "Road Book/Petitioned ROW") {
                        var bookid = contentArray[i].feature.attributes.OBJECTID;
                        att = contentArray[i].feature.attributes;
                        content += "<span title='Road Book/Petitioned ROW - Click to highlight and expand' class='nored book' id='i" + att.OBJECTID + "' onclick='getit(" + att.OBJECTID + ",17,pet" + (att.OBJECTID).toString() + ");'></span> " + "<a target='_blank' href='https://recording.mesacounty.us/Landmarkweb//search/DocumentBy?ClerkFileNumber=" + att.MAP + "'><b>" + att.MAP + "</b> (Road Book/Petitioned ROW)</a><br>" + "<div class='nored' id='pet" + (att.OBJECTID.toString()) + "' style='display:none;'>" + att.RBPG + "<br>" + att.COMMREC + "<br>Reception Number: <a target='_blank' href='https://recording.mesacounty.us/Landmarkweb//search/DocumentBy?ClerkFileNumber=" + att.REC_NO + "'>" + att.REC_NO + "</a>" + "<br>Road Plat 1: " + att.RDPLAT1 + "<br>Survey: " + att.Survey + "</div>";
                    ...
                    }else if (contentArray[i].layerName === "Digital Elevation Model") {
                        att = contentArray[i].feature.attributes;
                        content += "<br><b>" + att.Elevation_Ft + "<b>";
                    }else if (contentArray[i].layerName === "Contours 32000 50ft" ||
                              contentArray[i].layerName === "Contours 8000 20ft" ||
                              contentArray[i].layerName === "Contours 4000 10ft" ||
                              contentArray[i].layerName === "Contours 1000 2ft") {
                        att = contentArray[i].feature.attributes;
                        content += "<br><b>" + att.Contour + "<b>";
                    }
                }
