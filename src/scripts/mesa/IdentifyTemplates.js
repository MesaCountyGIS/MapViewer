...
//                    } else if (result.layerName === 'School District 51 Director Districts') {
        ...
        subList: function (evt, deferred, opt) {
            ...
            deferred.addCallback(function (response) {
                ...
                if (content !== '') {
                    aG.map.infoWindow.resize(400, 300);
                    aG.map.infoWindow.setTitle("Survey Documents");
                    aG.map.infoWindow.setContent(content);
                } else {
                    aG.map.infoWindow.resize(400, 300);
                    aG.map.infoWindow.setTitle("Survey Documents");
                    aG.map.infoWindow.setContent("No information available");
                }
                aG.map.infoWindow.show(evt.screenPoint, aG.map.getInfoWindowAnchor(evt.screenPoint));
            });
        },
...
function getit(objID, layerID, opt) {
    ...
    require(["esri/tasks/query", "esri/tasks/QueryTask", "esri/graphic", "mesa/graphicsTools"], function (Query, QueryTask, Graphic, graphicsTools) {
        var graphicTool = new graphicsTools({
            geometryServiceURL: aG.config.geometryServiceURL,
            mapRef: aG.map,
        });
